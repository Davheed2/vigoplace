...
//       <div className="hidden text-heading md:flex w-![100%] bg-white ">
...
//               <li key={nav.name} className="flex items-center relative cursor-pointer">
//                 <span>{nav.name}</span> <button className="flex items-center"><RiArrowDropDownLine /> <RiArrowDropUpLine /></button>
//                 <div className="absolute top-[30px] shadow px-[20px] py-[10px] bg-[#ffffff] z-[100] ">
//                   {nav.nameSubLinks?.map((name, index) => {
//                     return <div key={index} className="flex items-start justify-start ">
...
//                           return <li key={index}>
//                             <Link href={name.link}>{name.name}</Link>
//                           </li>
//                         })
//                         }
//                         </ul>
...
//                 <span>{nav.name}</span> <button><RiArrowDropDownLine/> <RiArrowDropUpLine/></button>
//               </li>
//             ))}
//           </ul>
//         </div>
...
export const MobileNav: React.FC<NavItemsProps> = ({ closeMenu }) => {
  const navs: MainNavLink[] = [
    //{ name: "Home", link: "/", nameSubLinks: [] },
    {
      name: "Why Vigoplace",
  ...
  const handleSubNavClick = (name: string) => {
    if (activeSubNav === name) {
      setActiveSubNav(null);
    } else {
  ...
  const [theSubLinkIndex, setTheSubLinkIndex] = useState<number>(0)

  return (
    <>
      <div className="px-4 pt-12 text-slate-900 rounded-[5px] bg-white md:bg-transparent overflow-y-auto max-h-[100vh] md:hidden">
        {navs.map((nav) => (
          <ul key={nav.name} className="bg-white relative flex flex-col mb-5">
            <li
              className="flex items-center justify-between cursor-pointer gap-0 w-full mb-7"
